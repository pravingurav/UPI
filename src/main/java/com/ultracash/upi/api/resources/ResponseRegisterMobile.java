...
@Path(ResourceURL.RESPONSE_REGISTER_MOBILE)
public class ResponseRegisterMobile {

	private static final Logger logger = LoggerFactory
			.getLogger(ResponseRegisterMobile.class);
	...
	public static Ack respHbt(@PathParam("tnxId") String txnid, String X) {
		RedisPersistence.getInstance().putString("RespRegMob" + txnid, X);
		logger.info("RespRegMob: " + X);

		String msgId = null;
		try {
			JAXBContext jaxbContext = JAXBContext
					.newInstance(RespRegMob.class);
			Unmarshaller unmarshaller = jaxbContext.createUnmarshaller();
			StringReader reader = new StringReader(X);
			RespRegMob respReqMob = (RespRegMob) unmarshaller
					.unmarshal(reader);
			msgId = respReqMob.getHead().getMsgId().toString();
